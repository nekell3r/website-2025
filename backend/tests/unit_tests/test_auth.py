...
def auth_service():
    ...
    with mock.patch('src.services.auth.redis_manager', new_callable=mock.AsyncMock) as mock_redis_manager:
        service = AuthService()
...
async def test_validate_russian_phone_valid(auth_service: AuthService):
    ...
    for phone_input, phone_expected_e164 in valid_phones:
        ...
        assert phonenumbers.format_number(phone_number_obj, phonenumbers.PhoneNumberFormat.E164) == phone_expected_e164
...
async def test_send_phone_code_success(auth_service: AuthService):
    ...
    with mock.patch.object(auth_service, 'test_send_sms', new_callable=mock.AsyncMock) as mock_send_sms:
        ...
        auth_service.redis.ttl.assert_called_once_with(f"rate_limit_{action}:{expected_e164}")
        auth_service.redis.set.assert_any_call(f"rate_limit_{action}:{expected_e164}", "1", expire=120)
        args_list = auth_service.redis.set.call_args_list
        code_set_call = next(call for call in args_list if call[0][0] == f"{action}:code:{expected_e164}")
...
async def test_send_phone_code_rate_limited(auth_service: AuthService):
    ...
    assert "Отправить код снова вы можете не ранее, чем через 60 секунд." in str(exc_info.value)
    expected_e164 = phonenumbers.format_number(parsed_phone_obj, phonenumbers.PhoneNumberFormat.E164)
    auth_service.redis.ttl.assert_called_once_with(f"rate_limit_{action}:{expected_e164}")
...
async def test_send_email_code_success(auth_service: AuthService):
    ...
    auth_service.redis.ttl.return_value = 0
    auth_service.redis.set.return_value = None
    with mock.patch.object(auth_service, 'test_send_mail', new_callable=mock.AsyncMock) as mock_send_mail:
        ...
        auth_service.redis.set.assert_any_call(f"rate_limit_{action}:{email_str}", "1", expire=120)
        args_list = auth_service.redis.set.call_args_list
...
async def test_send_email_code_rate_limited(auth_service: AuthService):
    ...
    assert "Отправить код снова вы можете не ранее, чем через 30 секунд." in str(exc_info.value)
    auth_service.redis.ttl.assert_called_once_with(f"rate_limit_{action}:{email_str}")

@pytest.mark.asyncio
async def test_send_registration_email_code_new_user(auth_service: AuthService, mock_db_dep: DBDep):
    data = EmailInput(email="new@example.com")
    mock_db_dep.users.get_one_or_none.return_value = None
    auth_service.redis.ttl.return_value = 0
    with mock.patch.object(auth_service, 'send_email_code', new_callable=mock.AsyncMock) as mock_send_email_code_method:
        ...
        mock_send_email_code_method.assert_called_once_with(data.email, action="registration")

@pytest.mark.asyncio
async def test_send_registration_email_code_existing_user(auth_service: AuthService, mock_db_dep: DBDep):
...
@pytest.mark.asyncio
async def test_send_registration_phone_code_new_user(auth_service: AuthService, mock_db_dep: DBDep):
    ...
    with mock.patch.object(auth_service, 'validate_russian_phone', return_value=parsed_phone_obj) as mock_validate, \
         mock.patch.object(auth_service, 'send_phone_code', new_callable=mock.AsyncMock) as mock_send_phone_code_method:
        ...
        assert result == {"status": "Ok"}
        mock_validate.assert_called_once_with('tel:+7-903-123-45-67')
        expected_e164 = phonenumbers.format_number(parsed_phone_obj, phonenumbers.PhoneNumberFormat.E164)
        mock_db_dep.users.get_one_or_none.assert_called_once_with(phone=expected_e164)
        mock_send_phone_code_method.assert_called_once_with(parsed_phone_obj, "registration")

@pytest.mark.asyncio
async def test_send_registration_phone_code_existing_user(auth_service: AuthService, mock_db_dep: DBDep):
...
@pytest.mark.asyncio
async def test_send_reset_phone_code_user_exists(auth_service: AuthService, mock_db_dep: DBDep):
...
@pytest.mark.asyncio
async def test_send_reset_phone_code_user_not_found(auth_service: AuthService, mock_db_dep: DBDep):
...
def mock_request_with_token():
    def _mock_request_with_token(access_token_val: str | None, refresh_token_val: str | None):
...
@pytest.mark.asyncio
async def test_get_current_user_payload_success_access_token(auth_service: AuthService, mock_request_with_token, mock_response_obj: Response):
    user_id = 10
    user_payload_dict = {"sub": str(user_id), "id": user_id, "is_super_user": False, "type": "access", "exp": datetime.now(timezone.utc) + timedelta(minutes=15)}
    request_mock = mock_request_with_token("fake_access_token", None)
    with mock.patch.object(auth_service, 'decode_access_token', return_value=user_payload_dict) as mock_decode_access, \
         mock.patch.object(auth_service, 'decode_refresh_token') as mock_decode_refresh:
        payload_dict_result = await auth_service.get_current_user_payload(request_mock, mock_response_obj)
...
@pytest.mark.asyncio
async def test_get_current_user_payload_success_refresh_token(auth_service: AuthService, mock_request_with_token, mock_response_obj: Response):
    user_id = 11
    refresh_payload_dict = {"sub": str(user_id), "id": user_id, "is_super_user": True, "type": "refresh", "exp": datetime.now(timezone.utc) + timedelta(days=7)}
    request_mock = mock_request_with_token("expired_or_invalid_access", "fake_refresh_token")
    with mock.patch.object(auth_service, 'decode_access_token', side_effect=AuthTokenExpiredServiceException(token_type="Access")) as mock_decode_access, \
         mock.patch.object(auth_service, 'decode_refresh_token', return_value=refresh_payload_dict) as mock_decode_refresh:
        payload_dict_result = await auth_service.get_current_user_payload(request_mock, mock_response_obj)
...
@pytest.mark.asyncio
async def test_get_current_user_payload_access_token_absent_refresh_absent(auth_service: AuthService, mock_request_with_token, mock_response_obj: Response):
...
@pytest.mark.asyncio
async def test_get_current_user_payload_access_expired_refresh_absent(auth_service: AuthService, mock_request_with_token, mock_response_obj: Response):
...
@pytest.mark.asyncio
async def test_get_current_user_payload_access_expired_refresh_expired(auth_service: AuthService, mock_request_with_token, mock_response_obj: Response):
    request_mock = mock_request_with_token("fake_access_token", "fake_refresh_token")
    with mock.patch.object(auth_service, 'decode_access_token', side_effect=AuthTokenExpiredServiceException(token_type="Access")) as mock_decode_access, \
         mock.patch.object(auth_service, 'decode_refresh_token', side_effect=AuthTokenExpiredServiceException(token_type="Refresh")) as mock_decode_refresh, \
...
@pytest.mark.asyncio
async def test_get_current_user_payload_access_invalid_no_refresh(auth_service: AuthService, mock_request_with_token, mock_response_obj: Response):
    request_mock = mock_request_with_token("invalid_access_token", None)
    with mock.patch.object(auth_service, 'decode_access_token', side_effect=AuthTokenInvalidServiceException(token_type="Access")) as mock_decode_access, \
...
@pytest.mark.asyncio
async def test_get_current_user_payload_access_expired_refresh_invalid(auth_service: AuthService, mock_request_with_token, mock_response_obj: Response):
    request_mock = mock_request_with_token("fake_access_token", "invalid_refresh_token")
    with mock.patch.object(auth_service, 'decode_access_token', side_effect=AuthTokenExpiredServiceException(token_type="Access")) as mock_decode_access, \
         mock.patch.object(auth_service, 'decode_refresh_token', side_effect=AuthTokenInvalidServiceException(token_type="Refresh")) as mock_decode_refresh, \
         pytest.raises(AuthTokenInvalidServiceException) as exc_info:
        await auth_service.get_current_user_payload(request_mock, mock_response_obj)
    mock_decode_access.assert_called_once_with("fake_access_token")
...
@pytest.mark.asyncio
async def test_set_new_password_after_reset_code_not_verified(auth_service: AuthService, mock_db_dep: DBDep):
