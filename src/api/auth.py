...
from src.schemas.users import UserAdd, UserRequestAdd, UserLogin, PhoneInput, PhoneWithCode, EmailInput, \
    PhoneWithPassword, UserWithHashedPassword, UserUpdate
...
@register.post(
    "/send_phone_code",
    summary="Отправка кода подтверждения регистрации на телефон"
)
...
@register.post(
    "/send_email_code",
    summary = "Отправка кода подтверждения регистрации на почту"
)
async def send_email_code(
    data: EmailInput = Body(openapi_examples={
            "1":
                {
                    "summary" : "number 1",
                    "value":{
...
async def verify_register(
    ...
    try:
        ...
        raise HTTPException(404, detail="Код для этого номера не существует, отправьте заново")
    ...
    if data.email:
        ...
        try:
            ...
            raise HTTPException(404, detail="Код для этого email не существует, отправьте заново")
...
async def send_reset_code(
    ...
    if user is None:
        raise HTTPException(400, detail="Пользователь с таким телефоном не зарегистрирован в системе")
...
async def set_password(
    ...
    hashed_password = AuthService().hash_password(data.password)
    await db.users.edit(UserWithHashedPassword(**user.model_dump(), hashed_password=hashed_password), telephone=data.phone)
    await db.commit()
