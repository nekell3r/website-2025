...
from src.schemas.payments import CreatePaymentRequest, CreatePaymentResponse, PaymentWebhookData, Purchase
...
async def create_payment(
    ...
    current_data = await db.purchases.get_one_or_none(product_id=data.product_id, user_id=user_id, status="Completed")
    if current_data:
    ...
    '''
    payment_url = HttpUrl(f"https://someservice.ru")
    payment_data = Purchase(user_id=user_id, product_id=data.product_id, email=data.email, invoice_id=invoice_id)
...
@router.post(
    "/webhook",
    summary="Получение вебхука от cloudpayments"
)
